...
(() => {
  ...
  function goEnd() {
    ...
    stage = 'end';
    window.scrollTo(0, 0);
    chaptersEls.forEach(el => el.classList.remove('is-active'));
    if (hasGSAP && !reduced) {
      const it = document.querySelector('#end .t-italic'),
            mn = document.querySelector('#end .t-main'),
            others = document.querySelectorAll('#end .intro-eyebrow, #end .cta, #end .end-hint');
      gsap.set([it, mn, ...others], { opacity: 0, y: 26 });
      gsap.timeline()
        .to(it, { opacity: 1, y: 0, duration: .7, ease: 'power3.out' })
        .to(mn, { opacity: 1, y: 0, duration: .9, ease: 'power3.out' }, '-=0.45')
        .to(others, { opacity: 1, y: 0, duration: .6, stagger: .14, ease: 'power2.out' }, '-=0.3');
    }
    // armer le clic-partout après un court délai (pour ne pas capter le clic « Terminer »)
    setTimeout(() => { if (stage === 'end') document.addEventListener('click', backHome, { once: true }); }, 700);
  }
